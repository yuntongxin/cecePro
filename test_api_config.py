...
if __name__ == "__main__":
    # 预配置（修改这里的配置）
    BASE_URL = "https://api.xiaomimimo.com"  # 修改为你的基础URL
    API_KEY = "your-api-key-here"  # 修改为你的API Key
    
    print(f"使用预配置: {BASE_URL}")
    print(f"请修改脚本中的 BASE_URL 和 API_KEY 变量")
    print("\n或者在下面输入配置：")
    
    # 让用户输入（可选）
    use_custom = input("使用自定义配置? (y/n, 默认n): ").strip().lower()
    
    if use_custom == 'y':
        base_url = input("请输入基础URL: ").strip()
